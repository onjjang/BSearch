...
const searchForAlphabeticalIndex = ([stringToFindIndexForFirstLetter], alphabetArray = generateAlphabetArray()) => {
    ...
    let recursiveBinarySearch = (array, letter, startIndex, endIndex) => {
        ...
        // if you are on an iteration where the startIndex is greater than the endIndex, 
        // first set the binarySearchResults variable to be -1 and then return false
            if (startIndex > endIndex){
                binarySearchResults = -1;
                return false;
            }
        //find middle index between the given start and end, 
        //hint: use Math.floor(your calculation for the middle index between start and end) 
        //as that rounds down to the nearest integer, you are not searching decimals
        // for example for array from 0 to 49 (length is 50), 
        //you calculate midpoint by writing let midIndex = Math.floor((0+49) / 2)
        /* CODE GOES HERE */
        let midIndex = Math.floor((startIndex + endIndex) / 2)
    

        //check if the value at midIndex is equal to the character being searched for, return true if so
        // before returning true inside the if statement, 
        //write the following line of code: binarySearchCharacterIndex = midIndex; 
        // this sets the variable up top to the index the character was found at so you can see the result in browser
        ...
        // determine the if/else condition that should house these two return statements that 
        //recursively call the same function after splitting the data in one direction or another
        // note that just like 2 < 4 will evaluate to true, 'a' < 'b' will also evaluate true,
        // javascript can compare letters just like numbers by their position in the alphabet
        // you need to write the if statement that will determine what condition needs to happen for you to search 
        //the lower half (start to middle) of the array rather than the upper half (middle to end) next
